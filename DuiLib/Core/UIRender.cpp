...
void CRenderEngine::DrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle, bool bSmoothing)
{
	ASSERT(::GetObjectType(hDC) == OBJ_DC || ::GetObjectType(hDC) == OBJ_MEMDC);
    ...
    // add by zel:
    // Used to handle smoothing for rendering text.
    if (bSmoothing)
      graphics.SetTextRenderingHint(Gdiplus::TextRenderingHint(
