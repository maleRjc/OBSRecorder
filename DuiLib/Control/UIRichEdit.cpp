...
CRichEditUI::~CRichEditUI() {
    // Add by zel:
    /* Fix bug: Crash since CPaintManagerUI::m_pCurrentCaretRichedit
       isn't NULL when it had been deleted.*/
    if (m_pManager->GetCurrentCaretRichEdit() == this) {
      m_pManager->SetNoneForCurrentCareRichEdit();
    }
    // end.
    if( m_pTwh ) {
        m_pTwh->Release();
...
void CRichEditUI::SetEnabled(bool bEnabled, bool use_default_disable_color) {
	if (m_bEnabled == bEnabled) return;

	if (m_pTwh && use_default_disable_color) {
    ...
    if( m_pTwh ) 
	{
      ...
      //end.
        ...
        if( m_bVScrollBarFixing )
		{
         ...
         // modified by zel :
        // It uses DuiLib::CRenderEngine to draw real texts.
        // CTxtWinHost is diable to draw text on transparent windows.
        CRenderEngine::DrawText(hDC, m_pManager, rc, GetText(), m_dwTextColor,
                                m_iFont, m_lTwhStyle, true);
    }
...
LONG CRichEditUI::CalculateHeight(int limitH, int& width) {
  if (!m_pTwh)
    return 0;
  SIZEL szExtent = {-1, -1};
  LONG nWidth = 0;
  LONG nHeight = limitH;
  m_pTwh->GetTextServices()->TxGetNaturalSize(
      DVASPECT_CONTENT, GetManager()->GetPaintDC(), nullptr, nullptr,
      TXTNS_FITTOCONTENT, &szExtent, &nWidth, &nHeight);
  width = nWidth;
  return nHeight;
}

LONG CRichEditUI::CalculateWidth(int limitW, int& height) {
  if (!m_pTwh)
    return 0;
  SIZEL szExtent = {-1, -1};
  LONG nWidth = limitW;
